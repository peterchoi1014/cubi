...
- MCP servers in the banner: the status line shows a count of MCP servers by state (connected / failed / not-loaded, plus disabled). If a server shows failed, check `~/.cubi/mcp.json`, auth headers/OAuth, and whether the server process or URL is reachable; run `/mcp` for the per-server breakdown. For common servers, `cubi mcp install <name>` writes a known-good template and validates it via a `tools/list` round-trip; see [`mcp/registry.md`](mcp/registry.md).
